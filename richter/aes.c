...
return_type aes_encrypt2( const uint8_t in[N_BLOCK], uint8_t  out[N_BLOCK], const aes_context ctx[1] )
{
    if( ctx->rnd )
    {
                         ...
                         int32_t n_block, uint8_t iv[N_BLOCK], const aes_context ctx[1] )
{
    ...
    {
        xor_block(iv, in);
        if(aes_encrypt2(iv, iv, ctx) != EXIT_SUCCESS)
            return EXIT_FAILURE;
...
void aes_encrypt2_128( const uint8_t in[N_BLOCK], uint8_t out[N_BLOCK],
...
void aes_encrypt2_256( const uint8_t in[N_BLOCK], uint8_t out[N_BLOCK],
