...
#define SX1272_MC2_SF72  0x70
#define SX1272_MC2_SF82  0x80
#define SX1272_MC2_SF92  0x90
#define SX1272_MC2_SF102 0xA0
#define SX1272_MC2_SF112 0xB0
#define SX1272_MC2_SF122 0xC0
...
#define SX1272_MC1_LOW_DATA_RATE_OPTIMIZE  0x01 // mandated for SF112 and SF122
...
static void writeReg (u1_t addr, u1_t data ) {
    pca8574_put_data(0b11111110);
    hal_spi(addr | 0x80);
    hal_spi(data);
    pca8574_put_data(0b11111111);
}
...
static void configLoraModem () {
    sf2_t sf = getSf(LMIC.rps);
        ...
        if ((sf == SF112 || sf == SF122) && getBw(LMIC.rps) == BW125) {
        ...
        writeReg(LORARegModemConfig2, (SX1272_MC2_SF72 + ((sf-1)<<4)) | 0x04);
#else
#error Missing CFG_sx1272_radio/CFG_sx1276_radio
...
static void configPower () {
    ...
    writeReg(RegPaDac, readReg(RegPaDac)|0x4);  //Possivelmente este registrador estava sendo escrito erroneamente. PA_BOOST não estava sendo habilitado.*/


    // check board type for BOOST pin
    //tx("\nESTE E O VALOR DO REGISTRADOR RegOcp");
    writeReg(RegOcp, 0x3F);   //Setando o limite de corrente para um nível maior do que necessitamos, de modo a garantir o correto funcionamento.
   // txh(readReg(RegOcp));  //Precisamos liberar pelo menos 120mAh, sendo que o padrão é liberar 100mAh.

    writeReg(RegPaConfig, 0x8F);
   // tx("\nESTE E O VALOR DO REGISTRADOR REGPACONFIG ");
   // txh(readReg(RegPaConfig));

    writeReg(RegPaDac, 0x87);  //Forçando a escrita do 0x17 para habilitar o PA_BOOST
    //tx("\nESTE E O VALOR DO REGISTRADOR REGPADAC ");
    //txh(readReg(RegPaDac)|0x4);
    //tx("\n");
...
static void txlora () {
    ...
    u1_t sf = getSf(LMIC.rps) + 6; // 1 == SF72
...
static CONST_TABLE(u2_t, LORA_RXDONE_FIXUP)[] = {
    ...
    [SF72]  =     us2osticks(0), // (   0 ticks)
    [SF82]  =  us2osticks(1648), // (  54 ticks)
    [SF92]  =  us2osticks(3265), // ( 107 ticks)
    [SF102] =  us2osticks(7049), // ( 231 ticks)
    [SF112] = us2osticks(13641), // ( 447 ticks)
    [SF122] = us2osticks(31189), // (1022 ticks)
};
